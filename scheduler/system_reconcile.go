...
// allocReconcilerSystem is used to determine the set of allocations that require
...
// service jobs.
type allocReconcilerSystem struct {
    ...
    // maximimum placements
    maxCount int
}


// NewAllocReconcilerSystem creates a new reconciler that should be used to determine
// the changes required to bring the cluster state inline with the declared jobspec
func NewAllocReconcilerSystem(logger log.Logger, allocUpdateFn allocUpdateType, batch bool,
    jobID string, job *structs.Job, deployment *structs.Deployment, existingAllocs []*structs.Allocation,
    taintedNodes map[string]*structs.Node, evalID string, maxCount int) *allocReconcilerSystem {
    return &allocReconcilerSystem{
...
func (a *allocReconcilerSystem) Compute() *reconcileResults {
...
func (a *allocReconcilerSystem) cancelDeployments() {
...
func (a *allocReconcilerSystem) handleStop(m allocMatrix) {
    for group, as := range m {
...
func (a *allocReconcilerSystem) markStop(allocs allocSet, clientStatus, statusDescription string) {
...
func (a *allocReconcilerSystem) markDelayed(allocs allocSet, clientStatus, statusDescription string, followupEvals map[string]string) {
    for _, alloc := range allocs {
        a.result.stop = append(a.result.stop, allocStopResult{
            alloc:             alloc,
            clientStatus:      clientStatus,
...
func (a *allocReconcilerSystem) computeGroup(group string, all allocSet) bool {
    ...
    nameIndex := newAllocNameIndex(a.jobID, group, a.maxCount, untainted.union(migrate, rescheduleNow))
...
func (a *allocReconcilerSystem) filterOldTerminalAllocs(all allocSet) (filtered, ignore allocSet) {
    if !a.batch {
        return all, nil
    }
...
func (a *allocReconcilerSystem) handleGroupCanaries(all allocSet, desiredChanges *structs.DesiredUpdates) (canaries, newAll allocSet) {
...
func (a *allocReconcilerSystem) computeLimit(group *structs.TaskGroup, untainted, destructive, migrate allocSet, canaryState bool) int {
...
func (a *allocReconcilerSystem) computePlacements(group *structs.TaskGroup,
    ...
    if existing >= a.maxCount {
        return place
    }

    // Add remaining placement results
    if existing < a.maxCount {
        for _, name := range nameIndex.Next(uint(a.maxCount - existing)) {
            place = append(place, allocPlaceResult{
                name:      name,
                taskGroup: group,
            })
...
// are canarying.
func (a *allocReconcilerSystem) computeStop(group *structs.TaskGroup, nameIndex *allocNameIndex,
    ...
    // Hot path the nothing to do case
    remove := len(untainted) + len(migrate) - a.maxCount
    if remove <= 0 {
        return stop
    }
    ...
    if len(migrate) != 0 {
        mNames := newAllocNameIndex(a.jobID, group.Name, a.maxCount, migrate)
...
func (a *allocReconcilerSystem) computeUpdates(group *structs.TaskGroup, untainted allocSet) (ignore, inplace, destructive allocSet) {
...
// the followupEvalID
func (a *allocReconcilerSystem) handleDelayedReschedules(rescheduleLater []*delayedRescheduleInfo, all allocSet, tgName string) {
...
func (a *allocReconcilerSystem) handleDelayedLost(rescheduleLater []*delayedRescheduleInfo, all allocSet, tgName string) map[string]string {
